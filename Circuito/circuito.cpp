    ...
    id_out(C.id_out)
{
    for(int i = 0; i < this->getNumPorts(); ++i){
        if(C.ports.at(i)!=nullptr) this->ports.at(i) = C.ports.at(i)->clone();
    }
}
...
{
    this->Nin_circ = 0;
    for(auto ptr : this->ports) delete ptr;
    this->ports.clear();
    this->out_circ.clear();
    this->id_in.clear();
    this->id_out.clear();
...
Circuito& Circuito::operator=(const Circuito& C)
{
    ...
    this->clear();
    this->Nin_circ = C.getNumInputs();
    this->ports.resize(C.getNumPorts(),nullptr);
    for(int i = 0; i < this->getNumPorts(); ++i){
        if(C.ports.at(i)!=nullptr) this->ports.at(i) = C.ports.at(i)->clone();
    }
...
void Circuito::resize(int NI, int NO, int NP)
{
  ...
  this->clear();
  this->Nin_circ = NI;
  this->ports.resize(NP,nullptr);
  this->out_circ.resize(NO,bool3S::UNDEF);
  this->id_in.resize(NP, std::vector<int>(0));
  this->id_out.resize(NO, 0);
}
...
void Circuito::setPort(int IdPort, const std::string& Tipo, int Nin)
{
  ...
  ptr_Porta pPorta(nullptr);
  if (Tipo == "NT") pPorta = new PortaNOT();
  else if (Tipo == "AN") pPorta = new PortaAND(Nin);
  else if (Tipo == "NA") pPorta = new PortaNAND(Nin);
  else if (Tipo == "OR") pPorta = new PortaOR(Nin);
  else if (Tipo == "NO") pPorta = new PortaNOR(Nin);
  else if (Tipo == "XO") pPorta = new PortaXOR(Nin);
  else pPorta = new PortaNXOR(Nin);
  delete this->ports.at(IdPort-1);
  this->ports.at(IdPort-1) = pPorta;
  this->id_in.at(IdPort-1).resize(Nin,0);
}
...
void Circuito::simular(const std::vector<bool3S>& in_circ)
{
  ...
  // INICIALIZACAO
  for(int i = 0; i < this->getNumPorts(); ++i){
      this->ports.at(i)->setOutput(bool3S::UNDEF);
  }
  // auxiliares
  ...
  do{
      ...
      for(int idPort = 1; idPort <= this->getNumPorts(); ++idPort){
          if(this->getOutputPort(idPort)==bool3S::UNDEF){
              std::vector<bool3S> in_port(this->getNumInputsPort(idPort)); // local
              for(int j = 0; j < this->getNumInputsPort(idPort); ++j){
                  // De onde vem a entrada?
                  id_orig = this->getIdInPort(idPort,j);
                  // Valor bool3S da entrada
                  in_port.at(j) = (id_orig>0)? this->getOutputPort(id_orig) : in_circ.at(-id_orig-1); //id_orig !=0 sempre por causa do valid()
              }
              // Simula a porta "id" com entradads in_port
              this->ports.at(idPort-1)->simular(in_port);
              //Calcula os criterios de parada do algoritmo
              if(this->getOutputPort(idPort)==bool3S::UNDEF){
                  tudo_def = false;
              }else{
                  alguma_def = true;
  ...
  for(int idOutput = 1; idOutput <= this->getNumOutputs(); ++idOutput){
      //De onde vem a saida?
      id_orig = this->getIdOutputCirc(idOutput);
      // Valor bool3S da saida
      this->out_circ.at(idOutput-1) = (id_orig>0)? this->getOutputPort(id_orig) : in_circ.at(-id_orig-1); //id_orig !=0 sempre por causa do valid()
  }
}
